...
export function TaskDetailSheet({ taskId, open, onOpenChange, onUpdate }: TaskDetailSheetProps) {
  ...
  const fetchTaskDetails = async (id: number) => {
    ...
    try {
      ...
      setTask(res.data);
      setSelectedAssignee(res.data.assigned_to || null);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] bg-zinc-950 border-zinc-800 p-0 flex flex-col gap-0">
        <DialogHeader className="px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
          ...
          <DialogTitle className="text-xl font-semibold text-zinc-100">{task.title}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <div className="rounded-md border border-zinc-800 bg-zinc-900/50 p-3 text-sm text-zinc-400 min-h-[80px]">
